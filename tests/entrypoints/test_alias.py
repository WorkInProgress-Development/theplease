...
import pytest
from theplease.entrypoints.alias import _get_alias
...
def test_get_alias(monkeypatch, mocker, py2,
    ...
    args = Mock(
        ...
        alias='please', )
    mocker.patch('theplease.entrypoints.alias.which', return_value=which)
    shell = Mock(app_alias=lambda _: 'app_alias',
                 instant_mode_alias=lambda _: 'instant_mode_alias')
    monkeypatch.setattr('theplease.entrypoints.alias.shell', shell)
