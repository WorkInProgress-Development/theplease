...
from theplease.rules import pacman_not_found
from theplease.rules.pacman_not_found import match, get_new_command
...
@pytest.mark.parametrize('command', [
    ...
    Command('pacman llc', 'error: target not found: llc'),
    Command('sudo pacman llc', 'error: target not found: llc')])
@patch('theplease.specific.archlinux.subprocess')
