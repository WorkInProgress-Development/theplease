...
import os
import pytest
from theplease.shells import Bash
...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('theplease.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias please=\'eval $(theplease $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'please': 'eval $(theplease $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}

    def test_app_alias(self, shell):
        assert 'please () {' in shell.app_alias('please')
        assert 'FUCK () {' in shell.app_alias('FUCK')
        assert 'theplease' in shell.app_alias('please')
        assert 'PYTHONIOENCODING' in shell.app_alias('please')

    def test_app_alias_variables_correctly_set(self, shell):
        alias = shell.app_alias('please')
        assert "please () {" in alias
        assert 'TF_SHELL=bash' in alias
        assert "TF_ALIAS=please" in alias
