...
def get_all_executables():
    ...
    tf_alias = get_alias()
    tf_entry_points = ['theplease', 'please']
...
class Cache(object):
    ...
    def _setup_db(self):
        ...
        cache_path = Path(cache_dir).joinpath('theplease').as_posix()
...
def get_installation_version():
    try:
        ...
        return version('theplease')
    except ImportError:
        import pkg_resources

        return pkg_resources.require('theplease')[0].version


def get_alias():
    return os.environ.get('TF_ALIAS', 'please')


@memoize
def get_valid_history_without_current(command):
    def _not_corrected(history, tf_alias):
        """Returns all lines from history except that comes before `please`."""
        previous = None
        for line in history:
